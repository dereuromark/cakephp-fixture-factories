...
- **`FactoryTransactionStrategy` keeps its eager default; `LazyFactoryTransactionStrategy` is the new opt-in.** Behavior on existing test suites is unchanged: `setupTest()` still wraps the primary test connection (`test` by default, configurable via the protected `$primaryConnection` property), so direct `$table->save($entity)`, `$table->delete($entity)` and raw inserts via `$connection->execute()` inside test methods are rolled back at teardown alongside Factory operations. Additional connections are still tracked lazily via `ensureTransaction()` from inside `BaseFactory::save()` / `saveMany()`, so multi-database setups only pay the transaction cost on connections they actually write to — the same as 1.x. **New in v2**: a dedicated `LazyFactoryTransactionStrategy` that disables the eager wrap on the primary connection too. Opt in by pointing `'fixtureStrategy'` at it when your tests persist exclusively through Factories and you want to skip the transaction on connections a given test never touches.
