...
- **Custom `belongsTo` joins stay protected across CakePHP 5.x.** For associations that use `foreignKey => false` plus custom join conditions, fixture-factories now uses CakePHP `5.4+` association join-key introspection when available, and keeps a conservative legacy fallback for CakePHP `5.0`-`5.3`. That means FK-in-`definition()` detection and auto-skip composition keep recognizing common uuid-style custom joins during the transition window.
