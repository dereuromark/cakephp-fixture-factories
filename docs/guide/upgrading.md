...
- **DummyGenerator backend now requires `johnykvsky/dummygenerator:^0.2.1`.** v2 forwards the float weight to `boolean()` directly (added in v0.2.1) instead of rounding it to an integer percentage, so `optional()` weights below 0.005 — e.g. `optional(0.001)` for 0.1% chance — actually fire at the requested rate instead of silently rounding to 0%. If you previously passed sub-1% weights to `optional()` on the dummy backend and saw "never fires", you'll now see real hits at the configured rate. Bumping to v0.2.1 is part of the v2 install.
