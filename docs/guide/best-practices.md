...
$customer = EasybillCustomerFactory::make()->persist();
...
## Know when to use `getEntity()` vs `persist()`
...
- **`persist()`** — write to the DB and return the persisted entities. Use for integration tests, controller tests, or anywhere the assertion runs a query against real rows.
...
ArticleFactory::make(5)->withAuthors(2)->persist();
...
{
    $articles = ArticleFactory::make()->withAuthors(2)->persist();
    // ...
}
```
...
- Pick `getEntity()` vs `persist()` deliberately; default to in-memory.
